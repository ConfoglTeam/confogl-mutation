...
{
    Msg(format("OnGameEvent_tank_spawn, %d\n", Director.IsTankInPlay() ? 1 : 0));
...
{
    Msg(format("OnGameEvent_tank_killed, %d\n", Director.IsTankInPlay() ? 1 : 0));
    if (lastSpitterLimit > 0.0) {
        Msg(format("Resetting Spitter Limit to %f\n", lastSpitterLimit));
...
function OnGameEvent_player_team(params)
{
	...
	{
        Msg(format("OnGameEvent_player_team, %d\n", Director.IsTankInPlay() ? 1 : 0));
        if (lastSpitterLimit > 0.0) {
            Msg(format("Resetting Spitter Limit to %f\n", lastSpitterLimit));
            Convars.SetValue("z_spitter_limit", lastSpitterLimit);
            lastSpitterLimit = 0;
